...
export default class UserService extends UtilityService {
  ...
  static createUser(data) {
    ...
    const query = UserQuery.insertUser([
      email,
      this.ucFirstStr(firstname),
      this.ucFirstStr(lastname),
      pass,
      moment,
      moment
    ]);
  ...
  static signinUser(data) {
    return db
      .sqlQuery(UserQuery.findUserBy({ email: data.email }))
      .then(result => {
        if (
          result.rows[0] &&
          bcrypt.compareSync(data.password, result.rows[0]["password"])
        ) {
  ...
  static findUser(decoded) {
    ...
    return db
      .sqlQuery(UserQuery.findUser({ email, userId, isAdmin }))
  ...
  static findBy(option, label) {
    ...
    return db.sqlQuery(UserQuery.findUserBy(option)).then(result => {
      ...
      return {
        statusCode: 404,
        message: `No user with ${label || field} found`
      };
    });
  }
      ...
      try {
        ...
        if (decoded) {
          return this.findUser(decoded).then(user => {
            return user
              ? decoded
              : { statusCode: 401, message: "User does not exist" };
          });
        }
  ...
  static updatePhotoURL(options) {
    return db
      .sqlQuery(UserQuery.editUser({ values: options, key: "photo" }))
      .then(result => {
        if (result.rows[0]) {
          const { password, ...user } = result.rows[0];
